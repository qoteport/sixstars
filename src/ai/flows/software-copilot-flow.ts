...
import { z } from 'zod';
import { softwareProductSchema, copilotRequestBodySchema, copilotResponseMessageSchema, CopilotRequestBody, CopilotResponseMessage } from '@/lib/types';
...
function convertToGenkitContent(ourContent: Array<{ text: string } | { product: any }>): any[] {
  return ourContent.map(part => {
    ...
    if ('product' in part) {
      // This might need adjustment based on how tools handle product data in prompts.
      // For now, let's represent it as text in history.
      return { text: `[Product: ${part.product.name}]` };
    }
...
- When you find relevant products, you MUST include the full product data in the response using the 'product' content type. Do not just list names.
- Provide informative comparisons when multiple products are found.
- Give specific recommendations based on user needs and explain your reasoning.
- Be helpful, knowledgeable, and concise.

RESPONSE STRUCTURE:
1. Start with a brief summary of what you found.
2. Compare products when multiple are relevant, highlighting key differences.
3. Provide clear recommendations with justifications.
4. Use product cards for each recommended software by including the product object.
...
Always base your answers on actual product data from our database.`,
      ...
      SoftwareCopilotLogger.error('AI model returned no structured output', { requestId, result });

      // This is a fallback if the structured output fails
      const textResponse = result.text || "I'm sorry, I couldn't generate a valid response. Please try rephrasing your question.";
      return {
          role: 'assistant',
