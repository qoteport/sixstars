...
function SoftwareForm({ product, categories, partnerId, onComplete }: { product?: SoftwareProduct, categories: SoftwareCategory[], partnerId: string, onComplete: () => void }) {
...
export default function PartnerManagementPage() {
    ...
    const softwareProductsQuery = useMemoFirebase(() => {
        ...
        return query(collection(firestore, 'softwareProducts'), where('partnerId', '==', partnerId));
    }, [firestore, partnerId]);
        ...
        <Fragment>
            <div className="space-y-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            A list of all your software products on the platform.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                                    ...
                                    <TableCell>
                                        ...
                                        <DropdownMenuContent align="end">
                                            ...
                                            <DropdownMenuItem asChild>
                                                <Link href={paths.partner.software(product.id)}>
                                                    <BarChart className="mr-2 h-4 w-4"/> View Analytics
                                                </Link>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem asChild>
                                                <Link href={paths.software(product.id)} target="_blank">
            ...
            <Sheet open={sheetState.open} onOpenChange={(open) => !open && handleCloseSheet()}>
                <SheetContent className="sm:max-w-2xl w-full">
                    {sheetState.view === 'edit' && categories && partnerId && <SoftwareForm product={sheetState.product} categories={categories} partnerId={partnerId} onComplete={handleCloseSheet} />}
