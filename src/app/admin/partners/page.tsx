...
import { MoreHorizontal, PlusCircle, Loader2, CheckCircle, Edit, X } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
function PartnerFormSheet({ partner, allSoftware, onComplete }: { partner?: Partner, allSoftware: SoftwareProduct[], onComplete: () => void }) {
    ...
    useEffect(() => {
        ...
        const fileInput = document.getElementById('logo-upload-admin') as HTMLInputElement;
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if (!finalLogoUrl) {
            console.error('Logo URL is required. Please provide a URL or upload an image.');
            setIsSubmitting(false);
            return;
        }
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col h-full">
                ...
                <div className="flex-grow overflow-y-auto p-1 -mx-1 space-y-4 py-6">
                    ...
                    <div>
                        <Label htmlFor="logo-upload-admin">Upload a Logo</Label>
                        <div className="mt-2 flex items-center gap-4">
                            <FormControl>
                                <Input id="logo-upload-admin" type="file" accept="image/*" className="flex-1" onChange={(e) => {
